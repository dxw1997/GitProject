...
#            torchvision.transforms.CenterCrop(96),
            ...
            torchvision.transforms.Normalize(mean=[0.7077172, 0.5913799, 0.54669064], 
...
for i in range(len(read_test_folder)):
    ...
    s = data_transform(im)
    pred = model_test(s.unsqueeze(0).cuda())[-1].cpu()
    pred = F.sigmoid(pred)
...
for i in range(len(read_test_folderP)):
    ...
    #save the images
    x1 = plt.imsave('./model/pred_threshold/' + fname + 'thrPrediction.png', s)
   ...
   # s2 =threshold_predictions_v(s2)
    ...
    #save the Images
    y1 = plt.imsave('./model/label_threshold/' + fname + 'thrLabel.png', s3)
